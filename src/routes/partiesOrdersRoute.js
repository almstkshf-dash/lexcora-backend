...
router.get("/", authenticateToken, paginationValidator, partiesOrdersController.getAllPartiesOrders);
...
router.get("/party/:partyId", authenticateToken, partyIdValidator, partiesOrdersController.getOrdersByPartyId);

// Get a specific party order by ID
// Now validates that id is a positive integer
router.get("/:id", authenticateToken, idValidator, partiesOrdersController.getPartyOrderById);

// Create a new party order
// Now validates all required fields and data types before processing
router.post("/", authenticateToken, partyOrderValidators.create, partiesOrdersController.createPartyOrder);

// Update a party order
// Now validates id and update fields
router.put("/:id", authenticateToken, partyOrderValidators.update, partiesOrdersController.updatePartyOrder);

// Delete a party order
// Now validates that id is a positive integer
router.delete("/:id", authenticateToken, idValidator, partiesOrdersController.deletePartyOrder);
